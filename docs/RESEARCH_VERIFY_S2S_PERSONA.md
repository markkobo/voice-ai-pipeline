...
1. **Weights are not open as of 2026-05-28.** `github.com/QwenLM/Qwen3-Omni` (the open repo) hosts only Qwen3-Omni-30B-A3B-Instruct / -Thinking — *not* 3.5-Omni. The Instruct model's README is explicit: only three preset speakers (`Ethan`, `Chelsie`, `Aiden`). There is no `reference_audio` parameter. Source: [HF model card](https://huggingface.co/Qwen/Qwen3-Omni-30B-A3B-Instruct).
...
| **Qwen3.5-Omni (Plus/Flash/Light)** | [CLAIM] open [/CLAIM] — API only as of 2026-05-28 | 10-language CN/EN | **YES — 10-30 s reference audio, zero-shot** | High per paper benchmarks; cross-lingual identity preserved | n/a (API) | ~234 ms (Flash) | The model EverHome eventually wants. Not deployable on-prem yet. arXiv 2604.15804. |
